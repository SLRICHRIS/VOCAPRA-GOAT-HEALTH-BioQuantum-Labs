#!/usr/bin/env python3
"""
VOCAPRA Streamlit App – Elite UI v4.3
Added: tuned palette, audio playback, and class-prototype embeddings (from vocapra_project/prototypes/).
"""
...
PROTOTYPES_DIR = ARTIFACT_DIR / "prototypes"  # folder for labeled prototype audio
PALETTE = {
    "neon": "#00f3ff",
    "accent": "#bc13fe",
    "muted": "#999999",
...
# -----------------------
# Grad-CAM + unsupervised metrics helpers
# -----------------------
...
# -----------------------
# Prototype computation (from local folder)
# -----------------------
@st.cache_resource(show_spinner=False)
def build_prototypes_from_dir(protos_dir: Path, model, target_frames=TARGET_FRAMES):
    """
    Scan prototypes_dir/<class>/*.wav and compute mean embedding per class (prototype).
    Returns dict class_name -> prototype_vector
    """
    ...
    for cls in sorted([d for d in protos_dir.iterdir() if d.is_dir()]):
        ...
        for wav in sorted(cls.glob("*.wav")):
            try:
                ...
                # try direct predict-to-embedding
...
# -----------------------
# Plot helpers (palette aware)
# -----------------------
def make_neon_plot(x, y, color=PALETTE["neon"], title="Waveform", sr=SR):
    fig, ax = plt.subplots(figsize=(9,2.8), dpi=120)
    fig.patch.set_alpha(0); ax.patch.set_alpha(0)
    ax.plot(x, y, color=color, linewidth=1.6, alpha=1.0)
    for n in range(1,5):
        ax.plot(x, y, color=color, linewidth=1.6 + n*0.7, alpha=0.12/n)
    ax.set_facecolor('none')
    ax.spines['top'].set_visible(False); ax.spines['right'].set_visible(False); ax.spines['left'].set_visible(False)
    ax.spines['bottom'].set_color('#444444'); ax.tick_params(axis='x', colors='#bbbbbb', labelsize=9)
    ax.set_yticks([]); ax.set_xlabel("Time (s)", color='#bbbbbb', fontsize=9, fontfamily='monospace')
    ax.set_title(title, color=PALETTE["fg"], fontsize=11)
    ax.grid(axis='x', color='#222222', linestyle='--', linewidth=0.4, alpha=0.25)
    plt.tight_layout()
    return fig
...
def plot_probability_bars(probs, idx_to_label, top_k=8):
    ...
    ax.set_yticks(y_pos); ax.set_yticklabels([l.upper() for l in top_labels[::-1]], fontsize=9, fontfamily='monospace', color=PALETTE["fg"])
    ax.set_xlabel("Probability", color='#bbbbbb', fontsize=9); ax.set_xlim(0,1.0); ax.invert_yaxis()
    ...
    for bar in bars:
        ...
        color = '#222' if width < 0.18 else '#fff'
...
def plot_gradcam_and_mfcc(fixed, cam, y, sr):
    ...
    ax_mfcc.set_xticks(np.linspace(0, T_frames-1, 5)); ax_mfcc.set_xticklabels([f"{t:.2f}s" for t in np.linspace(0, duration_s, 5)], color='#bbbbbb')
    ...
    ax_cam.set_xticks(np.linspace(0, T_frames-1, 5)); ax_cam.set_xticklabels([f"{t:.2f}s" for t in np.linspace(0, duration_s, 5)], color='#bbbbbb')
    ax_cam.set_yticks([])
    cbar = fig.colorbar(im, ax=[ax_mfcc, ax_cam], orientation='vertical', pad=0.02); cbar.set_label("Feature magnitude", color='#bbbbbb', fontsize=9)
    cbar.ax.yaxis.set_tick_params(color='#bbbbbb'); plt.setp(plt.getp(cbar.ax.axes,'yticklabels'), color='#bbbbbb')
    plt.tight_layout()
    return fig
...
# Unsupervised options in sidebar
...
st.markdown("<div class='label-small'>// Acoustic Event Recognition — playback & prototype similarity</div>", unsafe_allow_html=True)
...
# show primary card
st.markdown(f"""
<div style="padding:10px; border-left:4px solid {PALETTE['accent']}; background: rgba(10,15,25,0.6)">
 <div style="display:flex; justify-content:space-between; align-items:center;">
  <div><div style="color:{PALETTE['accent']}; font-weight:700">PRIMARY DETECTION</div>
  <div style="font-size:2rem;">{pred_label}</div></div>
  <div style="text-align:right"><div style="color:#aaa">CONFIDENCE</div><div style="font-family:monospace">{conf_top*100:05.2f}%</div></div>
 </div></div>
""", unsafe_allow_html=True)
...
# footer
st.markdown(f"<div class='fixed-footer'>&copy; 2025 Rights Reserved by BioQuantum Labs</div>", unsafe_allow_html=True)
